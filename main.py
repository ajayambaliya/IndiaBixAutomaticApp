...
async def send_pdfs_to_whatsapp_groups(pdf_files: Dict[str, List[str]], target_date: str = None, languages: List[str] = ["en", "gu"]) -> None:
    ...
    Args:
        pdf_files: Dictionary mapping language codes to lists of PDF file paths
        target_date: The date being processed (if single date)
        languages: List of language codes to send PDFs for
    """
    ...
    try:
        ...
        for pdf_path in all_pdfs:
            ...
            # Format: current_affairs_YYYY-MM-DD_gu.pdf
            current_date = target_date
            if not current_date:
                match = re.search(r'current_affairs_(\d{4}-\d{2}-\d{2})_gu\.pdf', os.path.basename(pdf_path))
                if match:
                    current_date = match.group(1)
            
            # Format date for caption (e.g., 2026-04-30 -> 30 April 2026)
            try:
                date_obj = datetime.strptime(current_date, "%Y-%m-%d") if current_date else datetime.now()
            ...
            # Send single PDF with its specific caption
            await send_pdfs_to_whatsapp(groups, [pdf_path], caption)
